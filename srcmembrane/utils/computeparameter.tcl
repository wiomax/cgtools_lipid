...
proc ::cgtools::utils::compute_gelcluster_list { system_specs topology } {
    ...
    set rcut_nconn_gl 50.0
    set rxycut_nconn_gl 12.0
    set rxy2cut_nconn_gl [expr $rxycut_nconn_gl * $rxycut_nconn_gl] 
    set box_vector [setmd box_l]
    set boxx [lindex $box_vector 0]
    set boxy [lindex $box_vector 1]
    puts "box_vector: $box_vector"
    #set p2order_threshould 0.7
    #fixed don't change
    set p2order_threshould_dopc 0.7
    #fixed don't change
    set p2order_threshould_dppc 0.8
    set ngelconn_threshould 4 
    #set p2orderdiff_threshould 0.15
    set p2orderdiff_threshould_dppc 0.15
    set p2orderdiff_threshould_difflip 0.15
    set p2orderdiff_threshould_dopc 0.15
	...
	# Compute neiborlist of each lipid
	# include GL bead neighbors
        set b_gl 2
        set partnum_gl_now [lindex $mol [expr $b_gl + 1]]
	set posvec1 [part $partnum_gl_now print pos]
        #puts "posvec1: $posvec1"
	set posx1 [lindex $posvec1 0] 
	set posy1 [lindex $posvec1 1] 
        set b_gl_neighborlist [ analyze nbhood $partnum_gl_now $rcut_nconn_gl ]
        #puts "$b_gl_neighborlist"
        foreach bneighbornow $b_gl_neighborlist {
            set partnum_gl_neighbornow $bneighbornow
	    set posvec2 [part $partnum_gl_neighbornow print pos]
            #puts "posvec2: $posvec2"
	    #get xy distance of the two molecules
	    set posx2 [lindex $posvec2 0] 
	    set posy2 [lindex $posvec2 1] 
	    set dxnow [expr abs($posx1-$posx2)]
            while { $dxnow > $boxx} {
	        set dxnow [expr $dxnow - $boxx]	
	    }
	    set dynow [expr abs($posy1-$posy2)]
            while { $dynow > $boxy} {
	        set dynow [expr $dynow - $boxy]	
	    }
            set distance2 [expr $dxnow*$dxnow + $dynow*$dynow ]
            #puts "distance2: $distance2"
            if {$distance2 <= $rxy2cut_nconn_gl} {
	...
	#puts "$neighborlist_now"
    	lappend glxyneighborlist $neighborlist_now
      } 
    }
    #puts "[llength $p2orderlist]"
    #puts "$p2orderlist"
    puts "[llength $glxyneighborlist]"
    puts "glxyneighborlist: $glxyneighborlist"

    ##get gelmollinklist
    set gelmollinklist 0
    unset gelmollinklist
    set imol 0
    foreach mol_neighborlist $glxyneighborlist {
	...
	unset molneigh_i
		...
		if { $p2order_threshould_i == $p2order_threshould_j } {
			...
			if {$imol< $n_moltype0} {
				set p2orderdiff_threshould $p2orderdiff_threshould_dopc	
			} else {
				set p2orderdiff_threshould $p2orderdiff_threshould_dppc
		        }
		} else {
	...
	lappend gelmollinknew $molneigh_i
	...
	set jmol_index  0
		...
		set gelflag_jmol [lindex [lindex $gelmollinklist $jmol] 0]
		if { $gelflag_jmol == 0 || $jmol == $imol } {
...
proc ::cgtools::utils::find_all_gel_molecules { gelclusterlist } {
	set all_gel_molecules_list {}
        ...
        for { set icluster 0 } { $icluster < $ncluster} {incr icluster} {
			...
			if { [llength $all_gel_molecules_list] == 0 } {
