...
namespace eval ::cgtools::analysis::stress_profile {
    variable av_stresses
    variable av_stresses_i 0
    variable verbose
    variable zbins
    variable zrange
    variable stress_cntfile
...
proc ::cgtools::analysis::stress_profile::resetav_stress_profile { } {
    # Do nothing because we want to average this over the entire simulation
    
}

proc ::cgtools::analysis::stress_profile::printav_stress_profile { } {
	...
	close $f_stressprof
        ...
        # backup <$outputdir/av_zstress$suffix> 
        incr stress_cntfile
        set newfile 0
        while { $newfile == 0 } {
                if { [file exists "$outputdir/av_zstress$suffix.$stress_cntfile"] } {
                        incr stress_cntfile
                } else {
			set newfile 1 
		}
        }
        catch { exec cp $outputdir/av_zstress$suffix  $outputdir/av_zstress$suffix.$stress_cntfile }
    }
}

proc ::cgtools::analysis::stress_profile::setup_stress_profile { args } {
	...
	{zrange.arg "100." "Range over which to calculate stress profile"}
    }
...
# ::cgtools::analysis::analyze_stress_profile --
# 
# Calculates the stress tensor through the bilayer
#
proc ::cgtools::analysis::stress_profile::analyze_stress_profile { } {
    variable av_stresses
    variable av_stresses_i
    variable zrange
    variable zbins
...
proc ::cgtools::analysis::stress_profile::approximate_centre { args } {
...
proc ::cgtools::analysis::stress_profile::average_height { args } {
